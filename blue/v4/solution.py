"""
Weather Translation Challenge - Solution (v4)
"""
...
def main():
    ...
    for arch, seed in models_spec:
        pt, pv, bv = train_one(
            seed, arch, train_df, test_df, val_mask, mean, std, stn_to_id, n_stations, epochs=100
        )
        preds_test_all.append(pt)
        preds_val_all.append(pv)
        val_best_list.append((arch, seed, bv))
        print(f"[{arch} seed={seed}] best val MSE (normalised): {bv:.5f}", flush=True)

    # Unweighted ensemble average in normalised space
    ...
    for arch, seed, bv in val_best_list:
        print(f"  [{arch} seed={seed}] best val (normalised MSE): {bv:.4f}")
